...
ap.add_argument('-t', '--training', required=True,
              help='path to the XML eye only')
ap.add_argument('-m', '--model', required=True,
              help='path to the dlibmodel')
args = vars(ap.parse_args)

# grab the default options for dlib shape predictor
print("[INFO] setting shape predictor options...")
options = dlib.shape_predictor_raining_options()
...
# the training data better, but could cause over fitting; values closer
# to 0 will help *our model generalize but will require us to have
# training data in the order of 1000s of datapoints
options.nu = 0.1

# the number of cascades used to train the shape predictor -- this
# parameter has a *dramatic* impact on both the *accuracy*.and.*output
...
# controls amount of "jitter" (i.e.,data argumentation) when training
...
# tell the dlib.shape predictor to be verbose and printout status
